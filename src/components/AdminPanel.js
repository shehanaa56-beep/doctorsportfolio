...
import { collection, addDoc, getDocs, doc, setDoc, deleteDoc, getDoc, onSnapshot, query, where, updateDoc } from 'firebase/firestore';
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    const q = query(col, where('type', '==', 'appointment'));
    const unsub = onSnapshot(q, (snap) => {
      const apps = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (b.createdAt?.toDate?.() || new Date(0)) - (a.createdAt?.toDate?.() || new Date(0)));
  ...
  async function deleteServiceById(id) {
    ...
    } catch (e) { console.error('Error deleting service', e); alert('Failed to delete service: ' + e.message); }
  }
  ...
  async function deleteFeature(id) {
    ...
    try {
      ...
      loadFeatures();
    } catch (e) { console.error('Error deleting feature', e); alert('Failed to delete feature: ' + e.message); }
  }
  ...
  async function deleteAppointment(id) {
    ...
    try {
      ...
      setMessage('Failed to delete appointment: ' + errorMsg);
      setTimeout(() => setMessage(''), 3000);
    }
  }
  ...
  return (
    <section className="admin-panel">
      ...
      <div className="admin-section glass-card">
        ...
        {message && <div style={{background:'#10b981', color:'white', padding:8, borderRadius:6, marginBottom:12}}>{message}</div>}
        <div style={{marginTop:12}}>
